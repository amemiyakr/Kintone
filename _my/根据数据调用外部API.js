(function() {
    ...
    // =================================================
    //格式：
    // app.record.create.change.****  15，16行
    // kintone.proxy  23行
    ...
    kintone.events.on(eventName, function(e) {
        ...
        var eventName = [
        "app.record.create.change.郵便番号_請求先",
        "app.record.edit.change.郵便番号_請求先"
    ];

    // 詳細画面_請求先
    kintone.events.on(eventName, function(e) {
        var url = 'http://api.zipaddress.net/?zipcode=' + e.record['郵便番号_請求先']['value'];
        // 外部API呼出
        kintone.proxy(url, 'GET', {}, {}).then(function(resp) {
            // 成功時
            // JSON文字列をJSONオブジェクトに変換
            ...
            if (obj['code'] === 200) {
                // ステータス正常時
                var record = kintone.app.record.get();
                record['record']['都道府県_請求先']['value'] = obj['data']['pref'];
                record['record']['市区町村_請求先']['value'] = obj['data']['city'];
                record['record']['町域名_請求先']['value'] = obj['data']['town'];
                kintone.app.record.set(record);
            } else {
                // ステータス異常時
                alert(obj['code'] + '：' + obj['message']);
